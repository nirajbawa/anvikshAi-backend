...
class ExpertAuthSerivce:
    ...
    async def onboarding(data: ExpertOnboardingSchema) -> dict:
        try:
            ...
            expert = await ExpertModel.find_one({
                "email": email,
                "onboarding": False
            })
            ...
            messages = (
                "You are an expert categorization agent. Your task is to analyze the given resume and bio, education, stream of education to determine the expert's primary domain(s) (ex: software engineering) of expertise. "
            ...
            token = create_jwt_token({"sub": email, "role": "expert", "verified": expert.verified}, os.getenv(
    ...
    async def sign_in(data: SignInSchema) -> dict:
        try:
            ...
            token = create_jwt_token({"sub": data.email, "role": "expert"}, os.getenv(
                "EXPERT_SECRET_KEY"), timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES))
