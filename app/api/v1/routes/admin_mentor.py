from fastapi import APIRouter, Depends, HTTPException, status, Query
from typing import Annotated
from app.schemas.auth_schema import Admin
from app.core.security import get_current_active_admin
from app.services.admin_mentor_service import AdminMentorService
from fastapi.responses import JSONResponse
from app.schemas.auth_schema import ExpertInviteSchema
from fastapi import BackgroundTasks

admin_mentor = APIRouter()


@admin_mentor.post("/")
async def invite_mentor(
    current_user: Annotated[Admin, Depends(get_current_active_admin)],
    background_tasks: BackgroundTasks,
    data: ExpertInviteSchema
):
    try:
        result = await AdminMentorService.invite_mentor(data, background_tasks)
        return JSONResponse(status_code=status.HTTP_200_OK, content=result)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
